...
GF_Reciprocal PROC						; rcx - a (элемент поля)
	...
	cycle:
		...
		loop cycle						; уменьшение rcx, условный переход к cycle
...
PolySum PROC							; rcx - адрес результата (sum)
	...
	; m5 - метка до начала цикла по перемнной i
	jl m5							; если r10 < 0, переход к метке m4
	cmp r12, 0						; сравнение j и 0
	jl m5							; if (j < 0) { переход к m4 }
	m1:
		...
		cmp r10, 0						; сравнение r10 и 0
		jge m1							; если r10 >= 0, переход к метке m1
	; далее дозапись "хвоста" одного из массивов
	; программа начнет исолнение не более одного цикла
	m5:
	cmp r10, 0
	; m6 - до начала следующего цикла
	jl m6
	m2:
		mov r11, [rdx+8*r10]			; r11 = a[i]
